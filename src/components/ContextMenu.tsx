...
export const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, onClose, onAction }) => {
  ...
  return (
    <div 
      ref={ref}
      className="absolute z-[99999] w-56 bg-white/90 backdrop-blur-xl border border-white/20 rounded-lg shadow-xl py-1.5 animate-in fade-in zoom-in-95 duration-100"
      style={{ top: y, left: x }}
    >
      {menuItems.map((item, index) => {
        ...
        return (
          <button
            ...
            className="w-full px-3 py-1.5 flex items-center gap-3 text-sm text-gray-700 hover:bg-blue-500 hover:text-white transition-colors text-left"
          >
            {item.icon}
